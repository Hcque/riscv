	...
	class RISCV {
		...
		// 		}
			...
			void serial() {
				int cc = 0;
				while(!mem->isEnd()) {
					std::cout <<  cc++ << " pc:" << reg.pc << std::endl;
					// std::cerr <<  cc++ << " pc:" << reg.pc << std::endl;
		...
		public:
			RISCV (memory *_mem, pipelineType _type) : reg(0, _mem), glb(), 
			IF(&reg), ID(&reg), EX(&reg), MA(&reg), WB(&reg) { mem = _mem; type = _type; }
